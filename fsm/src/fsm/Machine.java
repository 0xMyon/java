 ...
 * @param <R>
 */
public class Machine<T, R, TYPE extends Type<TYPE, T>>
implements Language.Naive<Machine<T, R, TYPE>, T, TYPE>, Function<List<T>, List<R>> {
	...
	/**
	 * {@link Factory} of underlying type system
	 */
	...
	protected Machine(final Type.Factory<TYPE, T> factory, final T... values) {
		...
		for (int i = 0; i < values.length; i++) {
			final State<T, R, TYPE> next = state(i == values.length - 1);
	...
	/**
	 * include a {@link Machine} with its {@link State} and {@link Transition} given
	 * a {@link Type} conversion
	 *
	 * @param that
	 * @param f    {@link Type} conversion
	 * @return {@link Map} from {@link State} of that to {@link State} of this
	 */
	private <X, XTYPE extends Type<XTYPE, X>> Map<State<X, R, XTYPE>, State<T, R, TYPE>> include(
			final Machine<X, R, XTYPE> that, final Function<XTYPE, TYPE> f) {
		final Map<State<X, R, XTYPE>, State<T, R, TYPE>> map = new HashMap<>();
		that.states.stream().forEach(s -> map.put(s, state()));
		that.transitions.stream().forEach(t -> transition(map.get(t.source()), f.apply(t.type()), map.get(t.target())));
		return map;
	}
	...
	private Map<State<T, R, TYPE>, State<T, R, TYPE>> clone(final Stream<State<T, R, TYPE>> stream,
			final State<T, R, TYPE> initial, final Predicate<State<T, R, TYPE>> isFinal) {
	...
	public Machine<T, R, TYPE> concat(final Machine<T, R, TYPE> that) {
		...
		final Map<Tuple<State<T, R, TYPE>, State<T, R, TYPE>>, State<T, R, TYPE>> map = result.include(
				...
				Tuple.left(initial), t -> Objects.nonNull(t.right) && t.right.isFinal()
				);
		...
		if (this.epsilon) {
			// I -> I2
			result.transition(result.initial(), map.get(Tuple.right(that.initial())));
		}
		if (that.epsilon) {
			// F1 -> F
			result.transition(
	...
	private Machine<T, R, TYPE> operation(final Machine<T, R, TYPE> that, final BooleanOperator op) {
		...
		final Map<Tuple<State<T, R, TYPE>, State<T, R, TYPE>>, State<T, R, TYPE>> map = result.include(
				...
				t -> op.apply(
						Objects.nonNull(t.left) && t.left.isFinal(), Objects.nonNull(t.right) && t.right.isFinal()
						)
				);
		...
		map.entrySet().forEach(e -> {
			...
			final Set<TYPE> inputs = Stream.of(source.left, source.right).filter(Objects::nonNull)
					.map(State::nextSymbols).reduce(Stream.of(), Stream::concat).distinct().collect(Collectors.toSet());
			...
			result.transition(
					map.get(source), factory.universe().minus(inputs.stream().reduce(factory.empty(), Type::unite)),
	...
	public Machine<T, R, TYPE> unite(final Machine<T, R, TYPE> that) {
		return operation(that, Boolean::logicalOr).identify();
	}
	...
	public Machine<T, R, TYPE> parallel(final Machine<T, R, TYPE> that) {
		final Machine<T, R, TYPE> result = new Machine<>(factory, this.hasEpsilon() && that.hasEpsilon());
		final Map<Tuple<State<T, R, TYPE>, State<T, R, TYPE>>, State<T, R, TYPE>> map = result.include(
				Sets.product(this.states.stream(), that.states.stream()), Tuple.of(this.initial(), that.initial()),
		...
		map.entrySet().forEach(e -> {
			final Tuple<State<T, R, TYPE>, State<T, R, TYPE>> source = e.getKey();
			source.left.next().forEach(
					t -> result.transition(map.get(source), t.type(), map.get(Tuple.of(t.target(), source.right)))
					);
			source.right.next().forEach(
					t -> result.transition(map.get(source), t.type(), map.get(Tuple.of(source.left, t.target())))
					);
		});
	...
	public Machine<T, R, TYPE> iterate() {
		...
		// F1 -> I1
		result.transition(this.finals.stream().map(s0::get).collect(Collectors.toSet()), s0.get(this.initial));
	...
	public Machine<T, R, TYPE> complement() {
		...
		result.states().forEach(
				s -> result.transition(
						s, s.nextSymbols().reduce(factory.empty(), Type::unite).complement(),
	...
	private Machine<T, R, TYPE> determinize() {
		...
		Algorithm.itterative(Set.of(this.initial), source -> {

			final Set<TYPE> inputs = source.stream().map(State::nextSymbols).reduce(Stream.of(), Stream::concat)
					.distinct().collect(Collectors.toSet());

			// for each partition of the states input
			return Type.partition(inputs).stream().map(x -> {
				final Set<State<T, R, TYPE>> target = source.stream()
						.map(s -> s.next(x).collect(Collectors.toSet()).stream()).reduce(Stream.of(), Stream::concat)
	...
	/**
	 * identify equal {@link State} it is required to be deterministic before
	...
	public boolean contains(final List<T> word) {
		...
		states.add(initial);
		for (final T t : word) {
			states = states.stream().map(s -> s.next(t)).reduce(Stream.of(), Stream::concat)
					.collect(Collectors.toSet());
		}
	...
	public List<R> apply(final List<T> word) {
		...
		for (final T t : word) {
			states = states.stream().map(s -> s.left.next(t, s.right)).reduce(Stream.of(), Stream::concat)
					.collect(Collectors.toSet());
		}
	...
	THAT convert(final FACTORY factory, final Function<TYPE, TYPE2> FUNCTION) {
		...
		final var s1 = result.include(this, FUNCTION.andThen(factory::letter));

		result.transition(result.initial, factory.epsilon(), s1.get(this.initial));

		final var newFinal = result.state(true);
	...
	public static class Factory<T, R, TYPE extends Type<TYPE, T>>
	implements Language.Naive.Factory<Machine<T, R, TYPE>, T, TYPE> {

		public Factory(final Type.Factory<TYPE, T> factory) {
			this.factory = factory;
			this.empty = new Machine<>(factory, false);
			this.epsilon = new Machine<>(factory, true);
		}

		private final Type.Factory<TYPE, T> factory;
		private final Machine<T, R, TYPE> empty;
		private final Machine<T, R, TYPE> epsilon;
		...
		@Override
		public Machine<T, R, TYPE> letter(final TYPE type) {
	...
	public Machine<T, R, TYPE> reverse() {
		...
		s0.forEach((a, b) -> {
			if (a != null)
				a.next().forEach(t -> result.transition(s0.get(t.target()), t.type(), b));
		});

		// null -> F1
		result.transition(s0.get(null), this.finals.stream().map(s0::get).collect(Collectors.toSet()));
	...
	private boolean hasLoops() {
		...
		while (!current.isEmpty()) {
			current = current.stream().map(s -> s.next().map(Transition::target)).reduce(Stream.of(), Stream::concat)
					.collect(Collectors.toSet());
	...
	public List<T> random(final Random random) {
		...
		final List<T> word = new Vector<>();
		State<T, R, TYPE> state = initial();
		while ((!state.isFinal() || random.nextBoolean()) && state.next().count() != 0) {
			final var ts = state.next().collect(Collectors.toSet());
			final var t = ts.stream().skip(random.nextInt(ts.size())).findFirst().orElseThrow();
